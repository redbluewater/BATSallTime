...
##adopted from old R code "Join_BATS_ALL_with_master.R" by Shuting before July 2022, updated by Shuting on Oct 20, 2023 to add more automatic functional and reduce the manual editing 

library(dplyr)
setwd("~/Desktop/temporary")

##add new CTD data first
#download new BATS or BIOSSCOPE bottle file from "ORIG CTD FROM BATS" folder
#open .dat with excel text to column to separate into columns
new<-read.csv("92213_2315_physf_QC.csv",header=F)
...
#add new columns
new$Program<-rep("BIOSSCOPE", nrow(new))  #change to "BATS" if merging new BATS cruises
...
master<-read.csv("BATS_BS_COMBINED_MASTER_2022.7.27.csv",header=T) #read in old master
