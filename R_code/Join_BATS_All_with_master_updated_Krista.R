...
#now make this a loop - go through one folder at a time
for (a in 1:length(D)) {
  ...
  ##put in a check here to make sure the data do not already exist in the discrete data file
  check = new$Niskin_ID[1]
    
  if (!is.na(match(check,discrete$ID))){
    warning("Something is wrong, this sample is already in discrete data file'")
  }
  rm(check)
  ...
  #now edit new file column name match master column name 
  #was this:
  ...
  #set up a double check here before moving on
...
write.csv(discrete_match,"newData_forDiscrete.2024.01.03.csv",row.names=F)
