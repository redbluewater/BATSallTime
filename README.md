...
Generally, how do we get from CTD data to a master bottle file with all the parameters from Ruth and the discrete data from different sources.

## After a cruise 
* The CTD data goes to Craig Carlson to serve as an archive; no work is done on these files.
* The BATS team processes the CTD data and posts it on Dropbox. Right now Craig and Rachel have access to the processed data on BIOS-SCOPE
...
* Discrete data comes in later, and that is where the R code comes in (see below)
